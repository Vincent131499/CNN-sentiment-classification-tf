...
with tf.Graph().as_default():
    ...
    with session.as_default():
        ...
        #                      embdedding_size = FLAGS.embedding_dim,
        ...
        #获取完整路径
        out_dir = os.path.abspath(os.path.join(os.path.curdir, 'runs_L2', timestamp))
        print('Writing to {}\n'.format(out_dir))
        ...
        print('epoch数目：', FLAGS.num_epochs)
        # print('batch数目：', len(list(batches)))
        # print('一个epoch对应%d个batch'%int(len(list(batches)))/FLAGS.num_epochs)
        #对每一个baych循环训练
